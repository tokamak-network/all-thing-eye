...
def get_report_period_presets() -> list:
    ...
    this_month_first_half_end = now.replace(day=15).strftime('%Y-%m-%d') if current_day >= 15 else now.strftime('%Y-%m-%d')
    presets.append({
        ...
        "blocks": [
            ...
            {"type": "divider"},
            {
                ...
                "element": {
                    ...
                    "action_id": "ai_input",
                    "options": [
                        {"text": {"type": "plain_text", "text": "✅ Yes - AI generates intelligent summaries"}, "value": "true"},
                        {"text": {"type": "plain_text", "text": "❌ No - Basic summaries only"}, "value": "false"}
                    ],
                    "initial_option": {"text": {"type": "plain_text", "text": "✅ Yes - AI generates intelligent summaries"}, "value": "true"}
                },
            ...
            },
            {
                ...
                "element": {
                    ...
                    "placeholder": {"type": "plain_text", "text": "Select channel to post report"},
...
async def generate_and_send_report(channel_id: str, start_date: str, end_date: str, use_ai: bool, user_id: str):
    ...
    async with httpx.AsyncClient(timeout=120.0) as client:
            ...
            json={
                ...
                "blocks": [
                    ...
                    },
                    {
                        "type": "context",
                        "elements": [
                            {"type": "mrkdwn", "text": f"_Requested by <@{user_id}>. This may take 30-60 seconds._"}
                        ]
                    }
                ]
            },
        ...
        try:
            ...
            from backend.api.v1.reports import (
                get_github_stats, get_active_projects, generate_project_summaries,
                generate_highlight, format_tech_stats_table, get_mongo
            )
            from src.report.external_data import (
                get_staking_data, get_staking_summary_text,
                get_ton_wton_tx_counts, get_transactions_summary_text,
                get_market_cap_data, get_market_cap_summary_text,
            )
                ...
                json={
                    ...
                    "blocks": [
                        ...
                        },
                        {
                            "type": "context",
                            "elements": [
                                {"type": "mrkdwn", "text": f"_Generated by <@{user_id}> using {'AI' if use_ai else 'basic'} summaries_"}
                            ]
                        }
                    ]
                },
...
async def slack_interactive(request: Request, background_tasks: BackgroundTasks):
    ...
    if payload.get("type") == "view_submission":
        ...
        if view.get("callback_id") == "schedule_modal":
            ...
            values = view["state"]["values"]
            period_value = values["period_block"]["period_input"]["selected_option"]["value"]
