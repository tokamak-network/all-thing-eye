...
class MemberCreate(BaseModel):
    ...
    recording_name: Optional[str] = (
        None  # Name used in meeting recordings (e.g., "YEONGJU BAK" for Zena)
    )
            ...
            try:
                ...
                if "member_identifiers" in collection_names:
                    ...
                    async for identifier in identifiers_cursor:
                        ...
                        logger.debug(
                            f"Member {member_id} identifier: source={source}, type={identifier_type}, value={identifier_value}"
                        )
                    ...
                    if identifier_list:
                        logger.debug(
                            f"Member {member_id} ({member.get('name')}) has {len(identifier_list)} identifiers: {identifiers}"
                        )
...
async def create_member(
    member_data: MemberCreate, request: Request, _admin: str = Depends(require_admin)
) -> Dict[str, Any]:
    """
    ...
    try:
            ...
            if "@" in member_data.slack_id:
                ...
                try:
                    ...
                    if slack_msg and slack_msg.get("user_id"):
                        ...
                        identifiers.append(
                            {"type": "slack_user_id", "value": actual_user_id}
                        )
                        logger.info(
                            f"Found Slack user_id {actual_user_id} for email {slack_email}"
                        )
                except Exception as e:
                    # If collection doesn't exist or query fails, just log and continue
                    logger.warning(
                        f"Could not find Slack user_id for {slack_email}: {e}"
                    )
                ...
                try:
                    ...
                    if notion_page:
                        ...
                        if notion_user_id:
                            ...
                            identifiers.append(
                                {"type": "notion_user_id", "value": notion_user_id}
                            )
                            logger.info(
                                f"Found Notion user_id {notion_user_id} for email {notion_email}"
                            )
                except Exception as e:
                    # If collection doesn't exist or query fails, just log and continue
                    logger.warning(
                        f"Could not find Notion user_id for {notion_email}: {e}"
                    )
        ...
        if member_data.recording_name:
            ...
            identifiers.append(
                {"type": "recording_name", "value": member_data.recording_name}
            )
            logger.info(
                f"Added recording_name '{member_data.recording_name}' for {member_data.name}"
            )
            ...
            for project_key in member_data.projects:
                ...
                if project_doc:
                    ...
                    if not member_exists:
                        ...
                        logger.info(
                            f"Added member {member_id} to project {project_key_normalized}"
                        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to create member: {str(e)}"
        )
    ...
    try:
        ...
        if update_data:
            ...
            await db["members"].update_one(
                {"_id": member_obj_id}, {"$set": update_data}
            )
        ...
        async def update_identifier(
            source: str, sub_type: str, identifier_value: Optional[str]
        ):
        ...
        if member_data.github_id is not None:
            ...
            old_github_id = (
                old_github_identifier.get("identifier_value")
                if old_github_identifier
                else None
            )
            ...
            if old_github_id and old_github_id != new_github_id:
                logger.info(
                    f"GitHub ID changed from '{old_github_id}' to '{new_github_id}' for member {member_id}"
                )
                ...
                if commits_result.modified_count > 0:
                    logger.info(
                        f"  Migrated {commits_result.modified_count} commits from '{old_github_id}' to '{new_github_id}'"
                    )

                # Migrate pull requests (author field)
                prs_result = await db["github_pull_requests"].update_many(
                    {"author": old_github_id}, {"$set": {"author": new_github_id}}
                )
                if prs_result.modified_count > 0:
                    logger.info(
                        f"  Migrated {prs_result.modified_count} PRs from '{old_github_id}' to '{new_github_id}'"
                    )
                ...
                if issues_result.modified_count > 0:
                    logger.info(
                        f"  Migrated {issues_result.modified_count} issues from '{old_github_id}' to '{new_github_id}'"
                    )

                total_migrated = (
                    commits_result.modified_count
                    + prs_result.modified_count
                    + issues_result.modified_count
                )
                if total_migrated > 0:
                            ...
                            if not duplicate_user_id:
                                ...
                                try:
                                    ...
                                    if "duplicate key" in str(
                                        e
                                    ).lower() or "E11000" in str(e):
                                        logger.warning(
                                            f"Duplicate key error when inserting Slack user_id {actual_user_id}: {e}"
                                        )
                                    else:
                                        raise
                            else:
                                logger.warning(
                                    f"Slack user_id {actual_user_id} already exists for another member"
                                )
                except Exception as e:
                    logger.warning(
                        f"Could not find Slack user_id for {slack_email}: {e}"
                                ...
                                if not duplicate_user_id:
                                    ...
                                    try:
                                        ...
                                        if "duplicate key" in str(
                                            ...
                                            logger.warning(
                                                f"Duplicate key error when inserting Notion user_id {notion_user_id}: {e}"
                                            )
                                        else:
                                            raise
                                else:
                                    logger.warning(
                                        f"Notion user_id {notion_user_id} already exists for another member"
                                    )
                except Exception as e:
                    logger.warning(
                        f"Could not find Notion user_id for {notion_email}: {e}"
        ...
        if member_data.recording_name is not None:
            await update_identifier(
                "recordings", "recording_name", member_data.recording_name
            )
            logger.info(
                f"Updated recording_name '{member_data.recording_name}' for member {member_id}"
            )
        ...
        if "projects" in update_data:
            ...
            removed_projects = set(old_projects_normalized) - set(
                new_projects_normalized
            )
            ...
            for project_key in removed_projects:
                ...
                if project_doc:
                    ...
                    logger.info(
                        f"Removed member {member_id} from project {project_key}"
                    )
            ...
            for project_key in added_projects:
                ...
                if project_doc:
                    ...
                    if not member_exists:
                        ...
                        logger.info(
                            f"Added member {member_id} to project {project_key}"
                        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to update member: {str(e)}"
        )


@router.get("/members/{member_id}")
async def get_member_detail(
    member_id: str, request: Request, _admin: str = Depends(require_admin)
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if not member:
            raise HTTPException(
                status_code=404, detail=f"Member not found: {member_id}"
            )
        ...
        github_identifiers = get_identifiers_for_member(
            member_mappings, "github", member_name
        )

        # Also get identifiers directly from member_identifiers collection
        github_identifier_docs = (
        ...
        all_github_identifiers = (
            set(github_identifiers) if github_identifiers else set()
        )
        ...
        or_conditions.append(
            {"author_name": {"$regex": f"^{member_name}$", "$options": "i"}}
        )
        ...
        logger.debug(
            f"GitHub commits query for {member_name}: {github_query}, all_identifiers: {all_github_identifiers}"
        )
        ...
        pr_or_conditions.append(
            {"author": {"$regex": f"^{member_name}$", "$options": "i"}}
        )
        ...
        issue_or_conditions = []
        if all_github_identifiers:
            issue_or_conditions.append(
                {"author": {"$in": list(all_github_identifiers)}}
            )
        ...
        if member_name:
            or_conditions.append(
                {"user_name": {"$regex": member_name, "$options": "i"}}
            )
        ...
        logger.debug(
            f"Slack query for {member_name}: {slack_query}, all_identifiers: {all_slack_identifiers}, email: {slack_email}"
        )
        ...
        notion_identifiers = get_identifiers_for_member(
            member_mappings, "notion", member_name
        )
        notion_query = {}
        ...
        drive_identifiers = get_identifiers_for_member(
            member_mappings, "drive", member_name
        )
        ...
        try:
            ...
            async def get_daily_counts_with_query(collection_name, query, date_field):
                try:
                    ...
                    logger.warning(
                        f"Trend aggregation failed for {collection_name} with query {query} and date_field {date_field}: {e}"
                    )
                    import traceback
            ...
            github_trend = await get_daily_counts_with_query(
                "github_commits", github_query, "date"
            )
            slack_trend = await get_daily_counts_with_query(
                "slack_messages", slack_query, "posted_at"
            )
            notion_trend = await get_daily_counts_with_query(
                "notion_pages", notion_query, "created_time"
            )
            drive_trend = await get_daily_counts_with_query(
                "drive_activities", drive_query, "timestamp"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch member detail: {str(e)}"
        )
...
async def get_member_activities(
    ...
    try:
        ...
        sources_to_query = (
            [source_type] if source_type else ["github", "slack", "notion", "drive"]
        )
        ...
        for source in sources_to_query:
            if source == "github":
                ...
                async for page in (
                    pages.find(query).sort("created_time", -1).limit(source_limit)
                ):
                    created_time = page.get("created_time")
                    if isinstance(created_time, datetime):
                        timestamp_str = (
                            created_time.isoformat() + "Z"
                            if created_time.tzinfo is None
                            else created_time.isoformat()
                        )
                    else:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch member activities: {str(e)}"
        )
    ...
    try:
        ...
        for source in sources_to_query:
            if source == "github":
                ...
                async for activity in (
                    drive_activities.find(query).sort("timestamp", -1).limit(20)
                ):
                    timestamp_val = activity.get("timestamp")
                    if isinstance(timestamp_val, datetime):
                        timestamp_str = (
