...
class CollectionExportRequest(BaseModel):
    ...
    collections: List[
        Dict[str, str]
    ]  # [{"source": "main", "collection": "members"}, ...]
    start_date: Optional[str] = None
    end_date: Optional[str] = None
...
def get_project_repositories_from_mongodb(project_key: str) -> Set[str]:
    ...
    try:
        ...
        if not project:
            logger.warning(
                f"Project {project_key} not found in MongoDB, falling back to config.yaml"
            )
        ...
        if repositories:
            logger.info(
                f"Found {len(repositories)} repositories for {project_key} from MongoDB"
            )
            return set(repositories)
        else:
            # If repositories list is empty, might not be synced yet, fall back to config
            logger.warning(
                f"No repositories found for {project_key} in MongoDB, falling back to config.yaml"
            )
            return get_project_repositories_from_config(project_key)
    ...
    try:
        ...
        while True:
            ...
            if response.status_code != 200:
                logger.warning(
                    f"GitHub API error {response.status_code}, falling back to config.yaml"
                )
                return get_project_repositories_from_config(project_key)
        ...
        logger.info(
            f"Found {len(repos)} repositories for {project_key} via GitHub Teams API"
        )
        return repos

    except Exception as e:
        logger.warning(
            f"Error fetching GitHub Teams data: {e}, falling back to config.yaml"
        )
        return get_project_repositories_from_config(project_key)
    ...
    try:
        ...
        for source in sources_needed:
            if source == "github":
                ...
                results.extend(
                    await fetch_drive_data(
                        db, members_to_filter, date_filter, member_info_map
                    )
                )
            elif source == "gemini":
                results.extend(
                    await fetch_gemini_data(
                        db, members_to_filter, date_filter, member_info_map
                    )
                )
...
async def fetch_github_data(
    ...
    if project and project != "all":
        ...
        if not project_repos:
            logger.warning(
                f"No repositories found for project {project}, returning empty results"
            )
            return []
            ...
            results.append(
                {
                    ...
                    "type": activity.get(
                        "action", "activity"
                    ),  # Use action field (not activity_type)
                    "member_name": member_name,
                    "member_email": member_info.get("email"),
                    ...
                    "file_name": activity.get(
                        "doc_title"
                    ),  # Use doc_title field (not file_name)
                    "file_type": activity.get(
                        "doc_type"
                    ),  # Use doc_type field (not mime_type)
        ...
        for daily in daily_docs:
            ...
            if members:
                ...
                for member in members:
                    for participant_name in participant_names:
                        if (
                            member.lower() in participant_name.lower()
                            or participant_name.lower() in member.lower()
                        ):
                            has_match = True
                            break
                    if has_match:
            ...
            try:
                ...
                if isinstance(timestamp, datetime):
                    ...
                    timestamp_str = (
                        str(timestamp) if timestamp else daily.get("target_date", "")
                    )
                ...
                if analysis and "_raw" in analysis and not analysis.get("summary"):
                    try:
                        ...
                        logger.warning(
                            f"Failed to parse _raw field for recordings_daily: {e}"
                        )
...
async def fetch_recordings_data(
    db, members: List[str], date_filter: dict, member_info_map: dict
) -> List[dict]:
    ...
    try:
        ...
        if members:
            ...
            for name in members:
                or_conditions.append(
                    {"participants": {"$regex": name, "$options": "i"}}
                )
            query["$or"] = or_conditions
        ...
        recordings = list(
            recordings_col.find(query).sort("meeting_date", -1).limit(10000)
        )
        ...
        for recording in recordings:
            try:
                ...
                if isinstance(meeting_date, datetime):
                    timestamp_str = (
                        meeting_date.isoformat() + "Z"
                        if meeting_date.tzinfo is None
                        else meeting_date.isoformat()
                    )
                else:
                ...
                if members:
                    for member in members:
                        for participant in participants:
                            if (
                                member.lower() in participant.lower()
                                or participant.lower() in member.lower()
                            ):
                ...
                primary_member = (
                    ...
                    else (participants[0] if participants else "Unknown")
                )
...
async def export_custom_data(
    ...
    try:
        logger.info(
            f"[EXPORT] Request body: members={body.selected_members}, fields={body.selected_fields}, sources={body.sources}"
        )
        ...
        for source in sources_needed:
            source_data = []
            if source == "github":
                source_data = await fetch_github_data(
                    db, members_to_filter, date_filter, member_info_map, body.project
                )
            elif source == "slack":
                source_data = await fetch_slack_data(
                    db, members_to_filter, date_filter, member_info_map
                )
            elif source == "notion":
                ...
                source_data = await fetch_gemini_data(
                    db, members_to_filter, date_filter, member_info_map
                )
            elif source == "recordings":
        ...
        if not results:
            logger.warning(
                f"No data found with filters - Members: {members_to_filter}, Sources: {sources_needed}, Results by source: {source_results_count}"
            )
            raise HTTPException(
                status_code=404,
...
async def get_custom_export_collections(request: Request):
    ...
    try:
        ...
        collections_by_source = {"main": [], "shared": [], "gemini": []}
        ...
        for name in main_collections:
            try:
                ...
                collections_by_source["main"].append(
                    {"name": name, "count": count, "source": "main"}
                )
            except Exception as e:
            ...
            for name in shared_collections:
                try:
                    ...
                    collections_by_source["shared"].append(
                        {"name": f"shared.{name}", "count": count, "source": "shared"}
                    )
                except Exception as e:
            ...
            for name in gemini_collections:
                try:
                    ...
                    collections_by_source["gemini"].append(
                        {"name": f"gemini.{name}", "count": count, "source": "gemini"}
                    )
                except Exception as e:
        ...
        return {
            ...
            "total_collections": sum(
                len(cols) for cols in collections_by_source.values()
            ),
        }
...
async def export_collection(request: Request, body: CollectionExportRequest):
    """
    ...
    if not body.collections or len(body.collections) != 1:
        raise HTTPException(
            status_code=400, detail="Must specify exactly one collection"
        )
    ...
    if not source or not collection_name:
        raise HTTPException(
            status_code=400, detail="Must specify source and collection"
        )
    ...
    try:
        ...
        if body.start_date or body.end_date:
            if source == "gemini" and actual_collection == "recordings_daily":
                ...
                if timestamp_field:
                    ...
                    if body.end_date:
                        try:
                            end_dt = datetime.fromisoformat(body.end_date) + timedelta(
                                days=1
                            )
                            date_filter["$lt"] = end_dt
                        except ValueError:
        ...
        # Generate filename
        date_suffix = (
            f"{body.start_date or 'all'}_{body.end_date or 'all'}"
            if (body.start_date or body.end_date)
            else "all"
        )
...
async def export_collections_bulk(request: Request, body: CollectionExportRequest):
    """
    ...
    try:
        ...
        with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
            ...
            for idx, collection_info in enumerate(body.collections, 1):
                logger.info(
                    f"Processing collection {idx}/{total_collections}: {collection_info.get('collection')}"
                )
                source = collection_info.get("source")
                collection_name = collection_info.get("collection")
                ...
                try:
                    ...
                    if collection_name.startswith("shared."):
                        ...
                        actual_collection = collection_name.replace("shared.", "")
                        source = (
                            "shared"  # Override source if collection name has prefix
                        )
                    elif collection_name.startswith("gemini."):
                        from backend.api.v1.ai_processed import get_gemini_db
                        ...
                        actual_collection = collection_name.replace("gemini.", "")
                        source = (
                            "gemini"  # Override source if collection name has prefix
                        )
                        db = None
                    elif source == "shared" or source == "other":
                        db = mongo.shared_async_db
                    ...
                    if body.start_date or body.end_date:
                        if (
                            source == "gemini"
                            and actual_collection == "recordings_daily"
                        ):
                            date_filter = {}
                            if body.start_date:
                                date_filter["$gte"] = body.start_date
                    ...
                    if body.format == "json":
                        file_content = json.dumps(
                            rows, indent=2, ensure_ascii=False, default=str
                        )
                        file_ext = "json"
                    elif body.format == "toon":
                        toon_data = {actual_collection: rows}
                    ...
                    clean_collection_name = (
                        re.sub(r"^(main|shared|gemini)\.", "", actual_collection)
                        if isinstance(actual_collection, str)
                        else actual_collection
                    )
                    date_suffix = (
                        f"_{body.start_date or 'all'}_{body.end_date or 'all'}"
                        if (body.start_date or body.end_date)
                        else ""
                    )
                    zip_filename = (
                        f"{source}_{clean_collection_name}{date_suffix}.{file_ext}"
                    )
                    ...
                    logger.info(
                        f"Added {zip_filename} to ZIP ({len(rows)} documents, {body.format.upper()} format)"
                    )

                except Exception as e:
                    logger.error(
                        f"Failed to export {collection_name}: {e}", exc_info=True
                    )
                    continue
        ...
        try:
            ...
            logger.info(
                f"ZIP file created successfully with {len(file_list)} files: {', '.join(file_list[:5])}{'...' if len(file_list) > 5 else ''}"
            )
...
async def export_members(
    request: Request, format: str = Query("csv", regex="^(csv|json|toon)$")
):
    """
