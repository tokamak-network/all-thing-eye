...
TOOLS = [
    ...
    },
    {
        ...
        "parameters": {
            "type": "object",
            "properties": {
                "source": {"type": "string", "enum": ["github", "slack", "all"], "description": "Data source"},
                "member_name": {"type": "string", "description": "Filter by member name (optional)"},
                "project_key": {"type": "string", "description": "Filter by project key (optional)"},
                "start_date": {"type": "string", "description": "ISO format date, e.g., '2025-12-01' (optional)"},
                "end_date": {"type": "string", "description": "ISO format date (optional)"},
    ...
    },
    {
        ...
        "parameters": {
            "type": "object",
            "properties": {
                "answer": {"type": "string", "description": "The final user-facing answer in Markdown."}
            },
            "required": ["answer"]
        }
    }
...
class MCPToolManager:
    ...
    async def get_team_members(args: Dict[str, Any]) -> Dict[str, Any]:
        db = get_mongo().db
        members = list(db["members"].find({}, {"_id": 0, "name": 1, "role": 1, "projects": 1}))
        return {"success": True, "data": {"members": members}}

    @staticmethod
    async def get_projects(args: Dict[str, Any]) -> Dict[str, Any]:
        db = get_mongo().db
        projects = list(db["projects"].find({}, {"_id": 0, "key": 1, "name": 1, "lead": 1}))
    ...
    async def get_activities(args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if args.get("start_date"):
            try: start_dt = datetime.fromisoformat(args["start_date"].replace('Z', '+00:00'))
            except: start_dt = datetime.strptime(args["start_date"], "%Y-%m-%d")
        
        end_dt = None
        if args.get("end_date"):
            try: end_dt = datetime.fromisoformat(args["end_date"].replace('Z', '+00:00'))
            except: end_dt = datetime.strptime(args["end_date"], "%Y-%m-%d")

        results = {}
        if source in ["github", "all"]:
            results["github"] = await fetch_github_activities(db, start_dt, end_dt, member_name, project_key, limit)
        if source in ["slack", "all"]:
            results["slack"] = await fetch_slack_messages(db, start_dt, end_dt, member_name, project_key, limit)
...
Your mission is to provide deep, accurate insights into team productivity.
            ...
            try:
                ...
                if not parsed["action"]:
                    ...
                    if iterations > 1: # Only fallback if it's not the first turn
                        return {"answer": ai_text, "iterations": iterations, "history": tool_history}
                    # If first turn and no action, prompt it again
                    conversation.append({"role": "user", "content": "Please follow the protocol: Thought then Action (JSON)."})
                    continue
                ...
                if tool_name == "final_answer":
                    ...
                    return {"answer": final_text, "iterations": iterations, "history": tool_history}
                ...
                tool_history.append({"tool": tool_name, "args": tool_args, "success": "error" not in observation})
                ...
                return {"error": str(e), "iterations": iterations, "history": tool_history}
            
    return {"answer": "죄송합니다. 최대 시도 횟수에 도달하여 답변을 드릴 수 없습니다.", "iterations": iterations, "history": tool_history}
