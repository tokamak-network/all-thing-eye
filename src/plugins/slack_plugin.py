...
class SlackPlugin(DataSourcePlugin):
    ...
    def collect_data(
        ...
        try:
            ...
            latest = (
                ...
                if end_date
                else str(datetime.now(tz=pytz.UTC).timestamp())
            )
            ...
            for idx, channel in enumerate(channels, 1):
                ...
                print(
                    f"   📂 [{idx}/{len(channels)}] Collecting from #{channel_name}...",
                    end=" ",
                )
            ...
            try:
                ...
                for msg in response["messages"]:
                    ...
                    message_data = {
                        ...
                        "is_thread_parent": "thread_ts" in msg
                        and msg.get("ts") == msg.get("thread_ts"),
                        "has_links": bool(
                            re.search(r"https?://[^\s]+", msg.get("text", ""))
                        ),
                    ...
                    if self.include_threads and message_data["is_thread_parent"]:
                        thread_messages = self._fetch_thread_replies(
                            channel_id, msg["thread_ts"], oldest, latest
                        )
    ...
    def _fetch_thread_replies(
        ...
        """Fetch replies in a thread (with date filtering and rate limit handling)"""
        replies = []
    ...
    def get_member_mapping(self) -> Dict[str, str]:
        ...
        for member in self.member_list:
            ...
            if slack_id and name:
                # Direct slack_id mapping (from member config)
    ...
    def get_member_details(self) -> Dict[str, Dict[str, str]]:
        ...
        for member in self.member_list:
            ...
            if name:
                details[name] = {"email": email, "slack_id": slack_id}
    ...
    def extract_member_activities(self, data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for reaction in data.get("reactions", []):
            ...
            activity = {
                ...
                "metadata": {"message_ts": message_ts, "emoji": emoji},
            }
            activities.append(activity)
