...
```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                           ALL-THING-EYE DATABASE SYSTEM                         │
└─────────────────────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────────────────────┐
│                              MAIN DATABASE (main.db)                            │
│                         [Member Index & Unified Activities]                     │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│  ┌──────────────┐         ┌─────────────────────┐        ┌──────────────────┐ │
│  │   members    │────────>│ member_identifiers  │<───────│ member_activities│ │
│  ├──────────────┤         ├─────────────────────┤        ├──────────────────┤ │
│  │ id (PK)      │         │ id (PK)             │        │ id (PK)          │ │
│  │ name ⭐      │         │ member_id (FK)      │        │ member_id (FK)   │ │
│  │ email        │         │ source_type         │        │ source_type      │ │
│  │ created_at   │         │ source_user_id ⚠️  │        │ activity_type    │ │
...
│  ┌──────────────────────────────────┘                                          │
│  │                                                                              │
│  │  ⭐ Primary identifier for all member queries                               │
│  │  ⚠️  Must match EXACT CASE from source systems                             │
│  │  🔑 UNIQUE constraint prevents duplicate activities                         │
│  │                                                                              │
│  │  ┌────────────────────┐                                                     │
│  │  │ data_collections   │                                                     │
│  │  ├────────────────────┤                                                     │
...
└─────────────────────────────────────────────────────────────────────────────────┘
                                       │
                                       │ Links to source databases via
                                       │ member_identifiers.source_type
                                       │
        ┌──────────────────────────────┼──────────────────────────────┐
        │                              │                              │
        ▼                              ▼                              ▼
...
┌─────────────────────┐   ┌─────────────────────┐   ┌──────────────────────────┐
│ GITHUB DB           │   │ SLACK DB            │   │ GOOGLE DRIVE DB          │
│ (github.db)         │   │ (slack.db)          │   │ (google_drive.db)        │
├─────────────────────┤   ├─────────────────────┤   ├──────────────────────────┤
│                     │   │                     │   │                          │
│ ┌─────────────────┐ │   │ ┌─────────────────┐ │   │ ┌──────────────────────┐ │
│ │ github_members  │ │   │ │ slack_channels  │ │   │ │ drive_activities     │ │
│ ├─────────────────┤ │   │ ├─────────────────┤ │   │ ├──────────────────────┤ │
│ │ id (PK)         │ │   │ │ id (PK)         │ │   │ │ id (PK)              │ │
│ │ login 🔑        │ │   │ │ name            │ │   │ │ timestamp            │ │
│ │ name            │ │   │ │ is_private      │ │   │ │ user_email 📧        │ │
│ │ email           │ │   │ │ member_count    │ │   │ │ action               │ │
│ └─────────────────┘ │   │ └─────────────────┘ │   │ │ event_name           │ │
│         │           │   │         │           │   │ │ doc_title            │ │
│         ▼           │   │         ▼           │   │ │ doc_type             │ │
│ ┌─────────────────┐ │   │ ┌─────────────────┐ │   │ │ doc_id               │ │
│ │ github_commits  │ │   │ │ slack_messages  │ │   │ └──────────────────────┘ │
│ ├─────────────────┤ │   │ ├─────────────────┤ │   │         │                │
│ │ id (PK)         │ │   │ │ ts (PK) 🕐      │ │   │         ▼                │
│ │ sha (UNIQUE)    │ │   │ │ channel_id (FK) │ │   │ ┌──────────────────────┐ │
│ │ message         │ │   │ │ user_id         │ │   │ │ drive_documents      │ │
│ │ author_login FK │ │   │ │ text            │ │   │ ├──────────────────────┤ │
│ │ repository_name │ │   │ │ thread_ts       │ │   │ │ doc_id (PK)          │ │
│ │ committed_at 📅 │ │   │ │ posted_at       │ │   │ │ title                │ │
│ │ additions       │ │   │ │ has_links       │ │   │ │ doc_type             │ │
│ │ deletions       │ │   │ │ has_files       │ │   │ │ first_seen           │ │
│ └─────────────────┘ │   │ └─────────────────┘ │   │ │ last_activity        │ │
│         │           │   │     │       │       │   │ └──────────────────────┘ │
│         ▼           │   │     ▼       ▼       │   │                          │
│ ┌─────────────────┐ │   │ ┌─────┐ ┌────────┐ │   │ ┌──────────────────────┐ │
│ │github_commit_   │ │   │ │slack│ │ slack_ │ │   │ │ drive_folders        │ │
│ │     files       │ │   │ │react│ │ links  │ │   │ ├──────────────────────┤ │
│ ├─────────────────┤ │   │ │ions │ │        │ │   │ │ folder_id (PK)       │ │
│ │ id (PK)         │ │   │ ├─────┤ ├────────┤ │   │ │ folder_name          │ │
│ │ commit_sha (FK) │ │   │ │emoji│ │url     │ │   │ │ parent_folder_id FK  │ │
│ │ filename        │ │   │ │user │ │type 🔗 │ │   │ │ project_key 🏷️      │ │
│ │ additions       │ │   │ │msg  │ │res_id  │ │   │ │ is_project_root      │ │
│ │ deletions       │ │   │ └─────┘ └────────┘ │   │ └──────────────────────┘ │
│ │ patch           │ │   │     │               │   │         │                │
│ └─────────────────┘ │   │     ▼               │   │         ▼                │
│                     │   │ ┌─────────────────┐ │   │ ┌──────────────────────┐ │
│ ┌─────────────────┐ │   │ │ slack_files     │ │   │ │drive_folder_members  │ │
│ │github_pull_     │ │   │ ├─────────────────┤ │   │ ├──────────────────────┤ │
│ │   requests      │ │   │ │ id (PK)         │ │   │ │ id (PK)              │ │
│ ├─────────────────┤ │   │ │ message_ts (FK) │ │   │ │ folder_id (FK)       │ │
│ │ id (PK)         │ │   │ │ channel_id (FK) │ │   │ │ user_email           │ │
│ │ number          │ │   │ │ name            │ │   │ │ access_level         │ │
│ │ title           │ │   │ │ filetype        │ │   │ └──────────────────────┘ │
│ │ state           │ │   │ │ uploaded_by     │ │   │                          │
│ │ author_login FK │ │   │ └─────────────────┘ │   │ ┌──────────────────────┐ │
│ │ repository_name │ │   │         │           │   │ │ NOTION DB (TBD)      │ │
│ │ merged_at       │ │   │         ▼           │   │ │ (notion.db)          │ │
│ └─────────────────┘ │   │ ┌─────────────────┐ │   │ └──────────────────────┘ │
│                     │   │ │ slack_users     │ │   │                          │
│ ┌─────────────────┐ │   │ ├─────────────────┤ │   │                          │
...
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│  🔑 Unique Constraints    ⭐ Primary Identifier    📧 Email Mapping             │
│  🕐 Timestamp Format      📅 Date Queries         🔗 Link Classification        │
│  🏷️  Project Association  ⚠️  Case Sensitive      FK = Foreign Key             │
│                                                                                 │
│  Data Flow:                                                                     │
│  1. Source DBs (GitHub, Slack, Drive) collect raw data                         │
│  2. member_identifiers maps source IDs to member names                         │
│  3. member_activities unifies all activities with activity_id deduplication    │
│  4. Reports query across all databases using member.name as key                │
│                                                                                 │
│  Cross-Database Links:                                                          │
│  • slack_links.link_type = 'github_pr' → github_pull_requests                  │
│  • slack_links.link_type = 'gdrive_doc' → drive_documents                      │
│  • slack_links.link_type = 'notion_page' → notion (future)                     │
│  • member_identifiers connects all sources via source_user_id                  │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```
